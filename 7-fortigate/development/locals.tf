...
locals {
  ...
  internal_ilb_address    = cidrhost(data.terraform_remote_state.networks_development.outputs.base_subnets_ips[0], 5)
  private_active_address  = cidrhost(data.terraform_remote_state.networks_development.outputs.base_subnets_ips[0], 3)
  private_network_gateway = cidrhost(data.terraform_remote_state.networks_development.outputs.base_subnets_ips[0], 1)
  private_passive_address = cidrhost(data.terraform_remote_state.networks_development.outputs.base_subnets_ips[0], 4)
  # keys regionX.name
  bu1_regions        = data.terraform_remote_state.bu1_development.outputs.bu_config[0].region_config
  ...
  bu1_primary_region_subnet = local.bu1_regions_ips.region1
...
data "terraform_remote_state" "bu1_development" {
  backend = "gcs"
