...
locals {
  ...
  internal_ilb_address    = cidrhost(data.terraform_remote_state.networks_nonproduction.outputs.base_subnets_ips[0], 5)
  private_active_address  = cidrhost(data.terraform_remote_state.networks_nonproduction.outputs.base_subnets_ips[0], 3)
  private_network_gateway = cidrhost(data.terraform_remote_state.networks_nonproduction.outputs.base_subnets_ips[0], 1)
  private_passive_address = cidrhost(data.terraform_remote_state.networks_nonproduction.outputs.base_subnets_ips[0], 4)
  # keys regionX.name
