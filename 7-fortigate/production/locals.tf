...
locals {
  vpc_private_network_project_id = data.terraform_remote_state.networks_production.outputs.base_host_project_id
  vpc_private_network            = data.terraform_remote_state.networks_production.outputs.base_network_name
  vpc_private_network_self_link  = data.terraform_remote_state.networks_production.outputs.base_network_self_link
  vpc_subnets_ips                = data.terraform_remote_state.networks_production.outputs.base_subnets_ips
  vpc_subnets_names              = data.terraform_remote_state.networks_production.outputs.base_subnets_names
  ...
  # keys regionX
  bu1_regions_ips           = data.terraform_remote_state.bu1_production.outputs.bu_config[0].base_projects[2].ip_ranges
...
# networks / production
data "terraform_remote_state" "networks_production" {
  backend = "gcs"
...
data "terraform_remote_state" "bu1_production" {
  backend = "gcs"
