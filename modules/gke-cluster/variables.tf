...
variable "default_node_tags" {
  description = "The network tags to be applied to default cluster nodes."
  type        = list(string)
  default     = []
}
...
variable "initial_node_count" {
  type        = number
  description = "Initial node count of a node pool"
  default     = 1
}

variable "default_node_machine_type" {
  type        = string
  description = "Default machine type of a node pool node"
  default     = "e2-medium"
}
...
variable "node_pools" {
  ...
  description = "Map of config for a node pool"
}
