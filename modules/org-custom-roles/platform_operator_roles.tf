...
locals {
  platform_operator_roles = {
    # Base Subset of Permissions curated from the following roles:
    # Organization Administrator - See Organization level config - Only the Read only permissions
    # Folder Administrator - See Folder level config - Only the Read only permissions
    # Organization Role Administrator - See Organization level IAM Configs - Only the read only permissions
    # Viewer - Read only access to all resources but not object level viewer
    # Compute.InstanceAdmin.v1 - Full control of Compute Engine instances, instance groups, disks, snapshots, and images. Read access to all Compute Engine networking resources. - For some day 2 tasks
    platform_operators_prod = {
      title       = "Platform Operators Production"
      description = "Platform Operators Production"
      role_id     = "platform_operators_prod"
      permissions = [
    ...
    platform_operators_nonp = {
      title       = "Platform Operators Non Production"
      description = "Platform Operators Non Production"
      role_id     = "platform_operators_nonp"
      permissions = [
        "accessapproval.requests.get",
        "accessapproval.requests.list",
        "accessapproval.settings.get",
