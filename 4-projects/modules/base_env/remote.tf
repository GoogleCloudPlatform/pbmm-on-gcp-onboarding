...
locals {
  ...
  perimeter_name                      = try(data.terraform_remote_state.network_env.outputs.restricted_service_perimeter_name, null)
...
data "terraform_remote_state" "business_unit_shared" {
  ...
  config = {
    ...
    prefix = "terraform/projects/business_units/shared"
  }
}
