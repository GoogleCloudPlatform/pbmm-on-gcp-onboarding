...
module "bu_folder" {
  ...
  for_each            = { for one_bu_config in local.bu_config : one_bu_config.business_code => one_bu_config if one_bu_config.is_enabled && local.identity_enabled }
  env                 = local.env
  remote_state_bucket = var.remote_state_bucket
...
module "env" {
  ...
  for_each                     = { for key, entry in local.flattened_bu_config_by_project : entry.prj_config.id => entry if entry.bu_config.is_enabled && local.identity_enabled }
  env                          = local.env
