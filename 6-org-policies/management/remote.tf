...
locals {
  ...
  restricted_enabled = try(data.terraform_remote_state.bootstrap.outputs.common_config.restricted_enabled, null)
  ...
  prj_m_shared_restricted       = try(data.terraform_remote_state.org.outputs.shared_vpc_projects["management"].restricted_shared_vpc_project_id, null)
  prj_m_shared_base             = try(data.terraform_remote_state.org.outputs.shared_vpc_projects["management"].base_shared_vpc_project_id, null)
  fldr_management               = try(data.terraform_remote_state.environments.outputs.env_folder, null)
}
...
data "terraform_remote_state" "environments" {
  ...
  config = {
    bucket = var.remote_state_bucket
    prefix = "terraform/environments/management"
  }
}
