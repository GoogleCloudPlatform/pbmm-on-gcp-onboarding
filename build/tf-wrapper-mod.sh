...
## terraform list resources from tfstate for single environment.
tf_list() {
  local path=$1
  local tf_env="${path#$base_dir/}"
  local tf_file
...
## terraform remove from tfstate resources for single environment.
tf_remove() {
  local path=$1
  local rlist=$(realpath "$resource_list")
  local tf_env="${path#$base_dir/}"
  local tf_file
  tf_file="$(convert_path "$tf_env")"
  echo "*************** TERRAFORM SHOW *******************"
  ...
  if [ -d "$path" ]; then
    ...
    if [ -f "$rlist" ]; then
      ...
      do
        terraform state rm "$p"  || exit 41
      done <  "$rlist"
    cd "$base_dir" || exit
    else
      echo "ERROR: list of resources ${rlist} does not exist"
    fi
  else
    echo "ERROR: ${path} does not exist"
  fi
